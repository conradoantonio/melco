...
class ApiController extends Controller
{
    ...
    public function processOrder(Request $req)
    {
        ...
        // dd($data->included[1]->attributes->total_pricing);
        // dd($data->included[1]->attributes->total_pricing);
        $total_pricing = $data->included[1]->attributes->total_pricing;
        $orden->shipping_cost = $total_pricing;
        $orden->save();
        
        $rate_id = $data->data->relationships->rates->data[0]->id;
        // dd($rate_id);

        $request_body = new \stdClass();
        $request_body->rate_id = (int)$rate_id;
