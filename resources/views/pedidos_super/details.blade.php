...
<section class="admin-content">
    ...
    <div class="printable-area">
        @include('pedidos_super.printable')
    </div>
</section>
<script type="text/javascript">
    ...
    function getPrintable(data, config) {
        ...
        var route = baseUrl.concat('/pedidos/supermarket/get-printable/'+item.id_pedido);
        ...
        if (! item.cancelacion ) {
            ...
            $('div.buttons').append('<button class="btn mr-2 btn-outline-danger cancel-row" data-row-id="'+item.id_pedido+'" > <i class="mdi mdi-cancel"></i>Cancelar orden</button>');
        } else if( item.id_tipo_pago == 1 &&  item.cancelacion && item.cancelacion.status == 1 ) {
            console.log('pasa a colocar boton reembolsar');
            $('div.buttons').append('<button class="btn mr-2 btn-outline-info refund-row" data-row-id="'+item.id_pedido+'" > <i class="mdi mdi-cash-refund"></i>Reembolsar pedido</button>');
        }
    }
</script>
@endsection