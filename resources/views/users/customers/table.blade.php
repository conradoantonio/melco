<table class="table table-hover table-sm data-table">
    ...
    <tbody>
        @foreach($items as $item)
            <tr>
                 <td class="align-middle" {{-- class="d-none" --}}>{{$item->id}}</td>
                ...
                <td class="text-center align-middle">
                    <a class="btn btn-dark btn-sm" href="{{url('usuarios/clientes/form/'.$item->id)}}" data-toggle="tooltip" data-placement="top" title="Editar"><i class="mdi mdi-square-edit-outline"></i></a>
                    @if( $item->status == 1 )
